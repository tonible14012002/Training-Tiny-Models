...
async def batch_generate_examples(prompt: str, llm_provider: LiteLLMProvider, count: int = 100, seed_msgs=[]) -> List[AIResponse]:
    formated_seed = "\n".join([msg for msg in seed_msgs])
    print(formated_seed)
    messages = [
        {
            "role": "system",
        ...
        {
            "role": "user", 
            "content": f"Convert following messages\n {formated_seed}"
        }
    ]
...
async def main():
    ...
    output_dir = f"training_data/payment_intent_generation/{int(now)}"
    os.makedirs(output_dir, exist_ok=True)

    # Read seed msgs from file

    with open('training_data/human_sms.json', 'r', encoding='utf-8') as f:
        seed_msgs = json.load(f)
        import random
        random.shuffle(seed_msgs)
    ...
    transform_text_to_payment = prompt_mgr.get_prompt("transform_text_to_payment_related")

    # Generate examples asynchronously
    tasks = []
    for i in range(num_of_calls):
        # subset seed messages for each call
        seed_subset = seed_msgs[i*examples_per_call:(i+1)*examples_per_call]
        task = batch_generate_examples(transform_text_to_payment, llm_model, examples_per_call, seed_msgs=seed_subset)
        tasks.append(task)
