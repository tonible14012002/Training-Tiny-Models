...
class TrainingOrchestrator:
    ...
    async def run(
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                checkpoint_path = self.trainer_service.get_item_path_by_id(current_checkpoint_id)
                ...
                combined_dataset = self._combine_versioned_files_to_dataset(versioned_files)
    ...
    def _combine_versioned_files_to_dataset(self, versioned_files: List[str]):
        ...
        for file_path in versioned_files:
            try:
                samples = self.data_manager.load_from_path(file_path)
                all_samples.extend(samples)
                self.logger.debug(f"Loaded {len(samples)} samples from {file_path}")
            except Exception as e:
